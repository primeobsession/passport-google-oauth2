...
function Strategy(options, verify) {
  ...
  if (url.pathname.indexOf('/userinfo') == (url.pathname.length - '/userinfo'.length)) {
	...
	console.log('HERE IS THE USER PROFILE', url);
}
...
Strategy.prototype.userProfile = function(req, accessToken, done) {
  ...
  this._userProfileURL = this._userProfileURL + seperator + 'userIp=' + userIp + '&quotaUser=' + userIp;

  var actualUrl = this._userProfileURL;
	
	console.log('ANOTHER look at the URL', this._userProfileURL);
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body, res) {
		...
		var payload = {
			message: 'GOOGLE_PROFILE_CALL',
			profile: actualUrl,
			userIp: userIp,
			path: (res && res.req) ? res.req.path : 'NONE Found in response',
			statusCode: (res && res.statusCode) ? res.statusCode : "NONE"
		}
